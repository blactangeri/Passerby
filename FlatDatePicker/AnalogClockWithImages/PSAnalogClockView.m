...
{
  return  [self initWithFrame:frame andImages:images withOptions:PSAnalogClockViewOptionSmoothHands];
}
...
- (void)start
{
  if (self.clockUpdateTimer) {
	...
	self.clockUpdateTimer = [NSTimer scheduledTimerWithTimeInterval:0.01
                                                           target:self
...
- (void)updateHoursHand
{
  ...
  double rotationForHoursComponent  = hoursFor12HourClock * degreesPerHour;
  double rotationForMinuteComponent = degreesPerMinute * [self minutes];
  
  double totalRotation = rotationForHoursComponent + rotationForMinuteComponent;
